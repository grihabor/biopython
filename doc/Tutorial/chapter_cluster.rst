...
-  | ``dist`` (default: ``'e'``, Euclidean distance)
   | Defines the distance function to be used (see
     `1 <#sec:distancefunctions>`__). Whereas all eight distance
...
`[subsec:distancematrix] <#subsec:distancematrix>`__).
...
`[subsec:clustercentroids] <#subsec:clustercentroids>`__ for a
description.

.. calculating-the-distance-between-clusters-1:
...
.. performing-hierarchical-clustering-1:
